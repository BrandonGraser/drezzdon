...
export default function BackgroundsPage({ images }: Props) {
    ...
    >
      ...
      <div className="pt-24 pb-8 px-6 text-center">
        <h1
          ...
          style={{ color: "#ff0000", fontSize: "clamp(2rem, 6vw, 4rem)", fontFamily: "'acumin-pro', sans-serif", fontWeight: 700, transform: "scaleY(1.2)", display: "inline-block" }}
        >
          backgrounds
        </h1>
      ...
      {/* GALLERY GRID */}
      <div className="px-4 pb-16 columns-2 md:columns-3 lg:columns-4 gap-3 space-y-3">
        {images.map((img) => (
          <div
            key={img}
            className="break-inside-avoid relative group overflow-hidden"
            style={{ border: "1px solid rgba(255,0,0,0.2)", cursor: "none" }}
          ...
          >
            ...
            >
              <span className="text-xs tracking-widest uppercase font-bold" style={{ color: "#ff0000", fontFamily: "'acumin-pro', sans-serif" }}>
                Download
              </span>
            </div>
          </div>
      ...
      {lightbox && (
        <div
          ...
          style={{ background: "rgba(0,0,0,0.92)", animation: "fadeIn 0.2s ease forwards", cursor: "none" }}
        ...
        >
          ...
          >
            <img
              src={`/backgrounds/${lightbox}`}
              alt={lightbox}
              className="max-w-full max-h-[75vh] object-contain"
              style={{ border: "2px solid #ff0000", animation: "popupIn 0.25s ease forwards" }}
            />
            <div className="flex gap-4">
              <a
                href={`/backgrounds/${lightbox}`}
                download={lightbox}
                className="px-8 py-3 text-xs tracking-[0.25em] uppercase font-bold transition-transform duration-200 hover:scale-105"
                style={{ background: "transparent", border: "2px solid #ff0000", color: "#ff0000", fontFamily: "'acumin-pro', sans-serif", cursor: "none" }}
                onMouseEnter={hoverOn} onMouseLeave={hoverOff}
              >
                Download
              </a>
              <button
                onClick={() => setLightbox(null)}
                className="px-8 py-3 text-xs tracking-[0.25em] uppercase font-bold transition-transform duration-200 hover:scale-105"
                style={{ background: "transparent", border: "2px solid rgba(255,255,255,0.3)", color: "rgba(255,255,255,0.5)", fontFamily: "'acumin-pro', sans-serif", cursor: "none" }}
                onMouseEnter={hoverOn} onMouseLeave={hoverOff}
              >
                Close
              </button>
